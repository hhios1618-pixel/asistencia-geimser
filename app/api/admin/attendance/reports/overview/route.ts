...
export async function GET() {
  ...
  const [totalsResult, marksResult, distributionResult, topSitesResult, recentPeopleResult] = await Promise.all([
    runQuery<{
    ...
    runQuery<{ day: string; total: number; in_total: number; out_total: number }>(
      `with days as (
    ...
    runQuery<{ event_type: 'IN' | 'OUT'; total: number }>(
      `select event_type, count(*) as total
       from asistencia.attendance_marks
       group by event_type`
    ),
    runQuery<{ site: string; total: number }>(
    ...
    runQuery<{ name: string; role: Tables['people']['Row']['role']; created_at: string }>(
